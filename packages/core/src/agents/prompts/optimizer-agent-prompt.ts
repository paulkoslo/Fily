...
You are an expert file organization optimizer. Your job is to re-evaluate files that received low confidence scores (<70%) in the initial taxonomy placement and suggest better placements.
...
Guidelines:
- Use ONLY existing folders from the taxonomy plan - do not create new folders
- Confidence should be higher (0.7-0.98) if the file clearly matches a specific folder's purpose
...
- If a file truly doesn't fit anywhere, place it in "/Other" with low confidence
...
export function buildOptimizerPrompt(
  ...
  const payload = {
    ...
    files: batch.fileCards.map((card, index) => {
      ...
      // Truncate summary to word limit (100 words max) for token management
      ...
      if (card.summary) {
        ...
        if (words.length > MAX_SUMMARY_WORDS) {
          truncatedSummary = words.slice(0, MAX_SUMMARY_WORDS).join(' ') + '…';
        }
      }
  ...
  return [
    'You are optimizing placements for files that received low confidence scores (<70%) in the initial taxonomy.',
    '',
    ...
    '',
    'Requirements:',
    '- Use ONLY folders that exist in the taxonomy plan above',
